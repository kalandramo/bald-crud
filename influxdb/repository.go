...
import (
	...
	"github.com/kalandramo/bald-crud/influxdb/field"
	"github.com/kalandramo/bald-crud/influxdb/filter"
	paging "github.com/kalandramo/bald-crud/influxdb/pagination"
	"github.com/kalandramo/bald-crud/influxdb/query"
	"github.com/kalandramo/bald-crud/influxdb/sorting"
...
// ListWithPaging 针对 storev1.PagingRequest 的列表查询（兼容 Query/OrQuery/FilterExpr）
func (r *Repository[DTO, ENTITY]) ListWithPaging(ctx context.Context, req *storev1.PagingRequest) ([]*DTO, int64, error) {
	if r.client == nil {
	...
	// apply filters
	var filterExpr *storev1.FilterExpr
	...
	req.FilteringType = &storev1.PagingRequest_FilterExpr{FilterExpr: filterExpr}
...
// ListWithPagination 针对 storev1.PaginationRequest 的列表查询
func (r *Repository[DTO, ENTITY]) ListWithPagination(ctx context.Context, req *storev1.PaginationRequest) ([]*DTO, int64, error) {
	if r.client == nil {
		return nil, 0, errors.New("influxdb database is nil")
	}
	if r.collection == "" {
	...
	req.FilteringType = &storev1.PaginationRequest_FilterExpr{FilterExpr: filterExpr}
