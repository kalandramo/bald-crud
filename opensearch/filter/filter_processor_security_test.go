...
func buildAndQuery(p *Processor, field string, op storev1.Operator, val string) map[string]any {
	expr := &storev1.FilterExpr{
		Type: storev1.ExprType_AND,
		Conditions: []*storev1.FilterCondition{
			{Field: field, Op: op, ValueOneof: &storev1.FilterCondition_Value{Value: val}},
		},
	}
...
func TestProcessor_SearchValueQuoted(t *testing.T) {
	p := NewProcessor()
	m := buildAndQuery(p, "name", storev1.Operator_SEARCH, "* OR admin_field:*")
...
func TestProcessor_EndsWithWildcardEscaped(t *testing.T) {
	p := NewProcessor()
	m := buildAndQuery(p, "name", storev1.Operator_ENDS_WITH, "x*?y")
	inner := extractMust(t, m)
