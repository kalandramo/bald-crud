...
func (sf StructuredFilter) BuildSelectors(builder *query.Builder, expr *storev1.FilterExpr) (*query.Builder, error) {
	if builder == nil {
	...
	var buildParts func(e *storev1.FilterExpr, b *query.Builder)
	buildParts = func(e *storev1.FilterExpr, b *query.Builder) {
		if e == nil {
			return
		}
		switch e.GetType() {
		case storev1.ExprType_EXPR_TYPE_UNSPECIFIED:
			return
...
func (sf StructuredFilter) buildCond(cond *storev1.FilterCondition) map[string]any {
	if cond == nil {
		return nil
	}
	field := cond.GetField()
	...
	case storev1.Operator_CONTAINS, storev1.Operator_ICONTAINS:
		if strings.TrimSpace(val) == "" {
			return nil
		}
		return map[string]any{"match_phrase": map[string]any{key: val}}
	case storev1.Operator_STARTS_WITH, storev1.Operator_ISTARTS_WITH:
		if strings.TrimSpace(val) == "" {
			return nil
		}
		return map[string]any{"prefix": map[string]any{key: val}}
	case storev1.Operator_ENDS_WITH, storev1.Operator_IENDS_WITH:
		if strings.TrimSpace(val) == "" {
			return nil
		}
	...
	case storev1.Operator_EXACT, storev1.Operator_IEXACT:
		return map[string]any{"term": map[string]any{key: val}}
