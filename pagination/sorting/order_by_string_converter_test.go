...
func TestOrderByStringConverter_Convert_JSON_SnakeCaseAndDirection(t *testing.T) {
	...
	if got[0].Field != "create_time" || got[0].Direction != storev1.Sorting_DESC {
		t.Fatalf("first sorting mismatch, want field=create_time dir=DESC, got: %#v", got[0])
	}
	if got[1].Field != "name" || got[1].Direction != storev1.Sorting_ASC {
...
func TestOrderByStringConverter_Convert_AIPFormat(t *testing.T) {
	...
	tests := []struct {
		name  string
		input string
		want  []*storev1.Sorting
	}{
		{
			name:  "dash prefix",
			input: "foo, bar desc",
			want: []*storev1.Sorting{
				{Field: "foo", Direction: storev1.Sorting_ASC},
				{Field: "bar", Direction: storev1.Sorting_DESC},
			},
		},
		{
			...
			input: "foo desc, bar",
			want: []*storev1.Sorting{
				{Field: "foo", Direction: storev1.Sorting_DESC},
				{Field: "bar", Direction: storev1.Sorting_ASC},
			},
		},
		{
