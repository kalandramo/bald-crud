...
// 并返回被移除的条件列表。
func RemoveExcludedConditions(filterExpr *storev1.FilterExpr, excludeFields []string) []*storev1.FilterCondition {
	...
	includeConditions := make([]*storev1.FilterCondition, 0, len(filterExpr.Conditions))
	excludeConditions := make([]*storev1.FilterCondition, 0, len(filterExpr.Conditions))
...
func ClearFilterExprByFieldNames(expr *storev1.FilterExpr, fieldName string) {
	if expr == nil {
		return
	}
...
func FilterFields(filterExpr *storev1.FilterExpr, excludeFields []string) []*storev1.FilterCondition {
	if filterExpr == nil || len(filterExpr.Conditions) == 0 {
		return []*storev1.FilterCondition{}
	}
