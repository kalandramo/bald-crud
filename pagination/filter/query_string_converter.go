...
func (qsc *QueryStringConverter) Convert(queryJSON string) (*storev1.FilterExpr, error) {
...
// 2. 对象：{"$and":[...], "$or":[...]}
func (qsc *QueryStringConverter) ParseQuery(queryJSON string) (*storev1.FilterExpr, error) {
...
func (qsc *QueryStringConverter) parseRawQuery(node *storev1.FilterExpr, raw any) error {
	switch v := raw.(type) {
	// 场景1：顶层是数组（默认AND逻辑）
	case []any:
		andFilterExpr := &storev1.FilterExpr{
			Type: storev1.ExprType_AND,
		}
	...
	case map[string]any:
		...
		if hasOr {
			...
			orFilterExpr := &storev1.FilterExpr{
...
func (qsc *QueryStringConverter) parseBaseCondition(node *storev1.FilterExpr, conditionMap map[string]any) error {
...
func (qsc *QueryStringConverter) addCondition(filterExpr *storev1.FilterExpr, op storev1.Operator, field string, value any) {
	filterExpr.Conditions = append(filterExpr.Conditions, &storev1.FilterCondition{
		Field:      field,
		Op:         op,
		ValueOneof: &storev1.FilterCondition_Value{Value: pagination.AnyToString(value)},
	})
}

func (qsc *QueryStringConverter) addJsonCondition(filterExpr *storev1.FilterExpr, op storev1.Operator, field, jsonPath string, value any) {
	filterExpr.Conditions = append(filterExpr.Conditions, &storev1.FilterCondition{
		Field:      field,
		Op:         op,
		JsonPath:   &jsonPath,
		ValueOneof: &storev1.FilterCondition_JsonValue{JsonValue: pagination.AnyToStructValue(value)},
	})
}

func (qsc *QueryStringConverter) addDatePartCondition(filterExpr *storev1.FilterExpr, op storev1.Operator, datePart storev1.DatePart, field string, value any) {
	filterExpr.Conditions = append(filterExpr.Conditions, &storev1.FilterCondition{
		Field:      field,
		Op:         op,
		DatePart:   &datePart,
...
func (qsc *QueryStringConverter) Equal(filterExpr *storev1.FilterExpr, field string, value any) {
	qsc.addCondition(filterExpr, storev1.Operator_EQ, field, value)
}

// MakeFieldFilter 构建一个字段过滤器
func (qsc *QueryStringConverter) MakeFieldFilter(filterExpr *storev1.FilterExpr, keys []string, value any) error {
	if len(keys) == 0 {
		return nil
	}
	...
	case 1:
		...
		if qsc.isJsonFieldKey(field) {
			...
			if len(jsonFields) == 2 {
				qsc.addJsonCondition(filterExpr, storev1.Operator_EQ, jsonFields[0], jsonFields[1], value)
				return nil
			}
	...
	case 2:
		...
		if qsc.isJsonFieldKey(field) {
			...
			if len(jsonFields) == 2 {
				...
				filterCondition.ValueOneof = &storev1.FilterCondition_JsonValue{JsonValue: pagination.AnyToStructValue(value)}
			} else {
				field = stringcase.ToSnakeCase(field)
				filterCondition.Field = field
				filterCondition.ValueOneof = &storev1.FilterCondition_Value{Value: pagination.AnyToString(value)}
			}
		} else {
			field = stringcase.ToSnakeCase(field)
			filterCondition.Field = field
