...
func TestConvertFilterByPagingRequest_WithQuery(t *testing.T) {
	...
	query := `{"name":"john"}`
	req := &storev1.PagingRequest{
		FilteringType: &storev1.PagingRequest_Query{
			Query: query,
		},
	}
	...
	// 验证返回的是 AND 类型表达式
	if result.Type != storev1.ExprType_AND {
...
func TestConvertFilterByPagingRequest_WithFilter(t *testing.T) {
	...
	req := &storev1.PagingRequest{
		FilteringType: &storev1.PagingRequest_Filter{
			Filter: filter,
		},
	}
...
func TestConvertFilterByPagingRequest_PriorityOrder(t *testing.T) {
		...
		Conditions: []*storev1.FilterCondition{
			{
				Field:      "priority",
				Op:         storev1.Operator_EQ,
				ValueOneof: &storev1.FilterCondition_Value{Value: "filterexpr"},
			},
		},
	}
	...
	if result.Type != storev1.ExprType_OR {
	...
	if cond.Op != storev1.Operator_EQ {
...
func TestConvertFilterByPagingRequest_InvalidFilter(t *testing.T) {
	...
	if result != nil {
		...
		if result.Type == storev1.ExprType_EXPR_TYPE_UNSPECIFIED {
...
func TestConvertFilterByPagingRequest_QueryJSONBField_DefaultEQ(t *testing.T) {
	...
	req := &storev1.PagingRequest{
		FilteringType: &storev1.PagingRequest_Query{Query: query},
	}
...
func TestConvertFilterByPagingRequest_QueryJSONBField_WithOperator(t *testing.T) {
	...
	for _, c := range conds {
		if c.Field == "profile" && c.GetJsonPath() == "name" {
			if c.Op != storev1.Operator_CONTAINS {
...
func collectAllConditions(expr *storev1.FilterExpr) []*storev1.FilterCondition {
	if expr == nil {
		return nil
	}
	out := make([]*storev1.FilterCondition, 0, len(expr.GetConditions()))
