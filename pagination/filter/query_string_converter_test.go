...
func dpPtr(dp storev1.DatePart) *storev1.DatePart {
	return &dp
}
...
func TestConvert_SimpleFieldAndOperator(t *testing.T) {
	...
	sortConditions := func(fe *storev1.FilterExpr) {
		...
		Conditions: []*storev1.FilterCondition{
			{
				Field:      "created_at",
				Op:         storev1.Operator_EQ,
				DatePart:   dpPtr(storev1.DatePart_YEAR),
				ValueOneof: &storev1.FilterCondition_Value{Value: "2023"},
			},
		},
	}
		...
		Conditions: []*storev1.FilterCondition{
			{
				...
				JsonPath: &jsonPath,
				ValueOneof: &storev1.FilterCondition_JsonValue{
				...
				Conditions: []*storev1.FilterCondition{
					{
						Field:      "x",
						Op:         storev1.Operator_EQ,
						ValueOneof: &storev1.FilterCondition_Value{Value: "1"},
					},
					{
						Field:      "y",
						Op:         storev1.Operator_EQ,
						ValueOneof: &storev1.FilterCondition_Value{Value: "2"},
					},
				},
				Groups: []*storev1.FilterExpr{
					{
...
func TestConvert_NestedArrayFlattening_SingleInnerArray(t *testing.T) {
	...
	sub := got.Groups[0]
	if sub.Type != storev1.ExprType_AND {
	...
	if cond.Field != "a" || cond.Op != storev1.Operator_EQ || cond.GetValue() != "1" {
...
func TestConvert_JsonFieldWithMoreThanTwoParts(t *testing.T) {
	...
	if cond.Op != storev1.Operator_CONTAINS {
