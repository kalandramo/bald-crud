...
func (fsc *FilterStringConverter) Convert(filterString string) (*storev1.FilterExpr, error) {
...
// mapOperator 映射 AIP 运算符到 storev1.Operator
func (fsc *FilterStringConverter) mapOperator(op string) storev1.Operator {
	switch op {
	case "=", "==":
		return storev1.Operator_EQ
	case "!=", "not":
		return storev1.Operator_NEQ
	case "<":
		return storev1.Operator_LT
	case "<=":
		return storev1.Operator_LTE
	case ">":
		return storev1.Operator_GT
	case ">=":
		return storev1.Operator_GTE
	case "isnull", "isNull", "is_null":
		return storev1.Operator_IS_NULL
	case "isnotnull", "isNotNull", "is_not_null":
		return storev1.Operator_IS_NOT_NULL
	case "contains":
		return storev1.Operator_CONTAINS
	case "startswith", "startsWith", "starts_with", ":":
		return storev1.Operator_STARTS_WITH
	case "endswith", "endsWith", "ends_with":
		return storev1.Operator_ENDS_WITH
	case "in":
		return storev1.Operator_IN
	case "notin", "notIn":
		return storev1.Operator_NIN
	default:
...
func (fsc *FilterStringConverter) invertOperator(op storev1.Operator) storev1.Operator {
	switch op {
...
func (fsc *FilterStringConverter) invertFilterExpr(fe *storev1.FilterExpr) {
	if fe == nil {
		return
	}

	// 切换组类型（德摩根律）
	if fe.Type == storev1.ExprType_AND {
		fe.Type = storev1.ExprType_OR
	} else if fe.Type == storev1.ExprType_OR {
		fe.Type = storev1.ExprType_AND
	}
...
func (fsc *FilterStringConverter) walk(out *storev1.FilterExpr, in *v1alpha1.Expr) {
	if in == nil {
		return
	}
	...
	case *v1alpha1.Expr_ConstExpr:
		// 处理常量表达式
		out.Conditions = append(out.Conditions, &storev1.FilterCondition{
			ValueOneof: &storev1.FilterCondition_Value{
	...
	case *v1alpha1.Expr_CallExpr:
		...
		if op == "and" || op == "or" {
			if op == "and" {
				out.Type = storev1.ExprType_AND
			} else {
				out.Type = storev1.ExprType_OR
			}

			for _, arg := range kind.CallExpr.Args {
				subExpr := &storev1.FilterExpr{}
			...
			if mappedOp == storev1.Operator_OPERATOR_UNSPECIFIED && fsc.err == nil {
				fsc.err = fmt.Errorf("unknown filter operator %q", op)
			}
			condition := &storev1.FilterCondition{
				Op: mappedOp,
			}
			...
			if len(kind.CallExpr.Args) >= 2 {
				...
				if constExpr, ok := kind.CallExpr.Args[1].ExprKind.(*v1alpha1.Expr_ConstExpr); ok {
					condition.ValueOneof = &storev1.FilterCondition_Value{
