...
func TestFilterStringConverter_ConvertComplex(t *testing.T) {
	...
	if got.Type != storev1.ExprType_AND {
		t.Fatalf("unexpected ExprType: got %v want %v", got.Type, storev1.ExprType_AND)
	}
	...
	getVal := func(c *storev1.FilterCondition) string {
		if c == nil || c.ValueOneof == nil {
			return ""
		}
		if v, ok := c.ValueOneof.(*storev1.FilterCondition_Value); ok {
			return v.Value
		}
		return ""
	}
	...
	if c0.Op != storev1.Operator_EQ {
		t.Fatalf("cond0 op = %v, want %v", c0.Op, storev1.Operator_EQ)
	}
	...
	if c1.Op != storev1.Operator_GT {
		t.Fatalf("cond1 op = %v, want %v", c1.Op, storev1.Operator_GT)
	}
	...
	if c2.Op != storev1.Operator_EQ {
		t.Fatalf("cond2 op = %v, want %v", c2.Op, storev1.Operator_EQ)
	}
...
func TestFilterStringConverter_mapOperator(t *testing.T) {
	...
	cases := []struct {
		...
		{"unknown", storev1.Operator_OPERATOR_UNSPECIFIED},
	}
...
func TestFilterStringConverter_Convert_AllOperators(t *testing.T) {
	...
	cases := []struct {
		...
		wantType *storev1.ExprType // nil 表示不检查 ExprType
	}{
		{"name = 'a'", storev1.Operator_EQ, nil},
		{"name != 'a'", storev1.Operator_NEQ, nil},
		{"age > 10", storev1.Operator_GT, nil},
		{"age >= 10", storev1.Operator_GTE, nil},
		{"age < 10", storev1.Operator_LT, nil},
		{"age <= 10", storev1.Operator_LTE, nil},
		{"name = 'a' OR name = 'b'", storev1.Operator_EQ, nil},
		{"NOT (name = 'a' OR name = 'b')", storev1.Operator_NEQ, nil},
		{"NOT (name = 'a' AND name = 'b')", storev1.Operator_NEQ, nil},
		{"in(name, 'a','b')", storev1.Operator_IN, nil},
		{"notin(name, 'x')", storev1.Operator_NIN, nil},
		{"contains(name, 'ex')", storev1.Operator_CONTAINS, nil},
		{"startsWith(name, 'pre')", storev1.Operator_STARTS_WITH, nil},
		{"endsWith(name, 'suf')", storev1.Operator_ENDS_WITH, nil},
		{"isNull(name)", storev1.Operator_IS_NULL, nil},
		{"isNotNull(name)", storev1.Operator_IS_NOT_NULL, nil},
		{"-name:*", storev1.Operator_NEQ, nil},
		{"name:*", storev1.Operator_STARTS_WITH, nil},

		// 组合逻辑：检查顶层 ExprType
		{"a = 1 AND b = 2", storev1.Operator_EQ, func() *storev1.ExprType { e := storev1.ExprType_AND; return &e }()},
		{"a = 1 OR b = 2", storev1.Operator_EQ, func() *storev1.ExprType { e := storev1.ExprType_OR; return &e }()},
	...
	for _, tc := range cases {
		...
		var condOp = storev1.Operator_OPERATOR_UNSPECIFIED
		found := false
