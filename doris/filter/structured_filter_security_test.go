	...
	for _, field := range hostileFields {
		expr := &storev1.FilterExpr{
			...
			Conditions: []*storev1.FilterCondition{
				{Field: field, Op: storev1.Operator_EQ, ValueOneof: &storev1.FilterCondition_Value{Value: "1"}},
			},
		}
...
func TestBuildFilterSelectors_JSONKeyMustBeValid(t *testing.T) {
	for _, jsonKey := range []string{`x' OR '1'='1`, `x'),('y`, `x'; --`} {
		expr := &storev1.FilterExpr{
			Type: storev1.ExprType_AND,
			Conditions: []*storev1.FilterCondition{
				{Field: "preferences." + jsonKey, Op: storev1.Operator_EQ, ValueOneof: &storev1.FilterCondition_Value{Value: "1"}},
			},
		}
		sf := NewStructuredFilter()
...
func TestBuildFilterSelectors_ValidFieldsStillWork(t *testing.T) {
	expr := &storev1.FilterExpr{
		Type: storev1.ExprType_AND,
		Conditions: []*storev1.FilterCondition{
			{Field: "name", Op: storev1.Operator_EQ, ValueOneof: &storev1.FilterCondition_Value{Value: "bob"}},
			{Field: "preferences.daily_email", Op: storev1.Operator_EQ, ValueOneof: &storev1.FilterCondition_Value{Value: "1"}},
		},
	}
