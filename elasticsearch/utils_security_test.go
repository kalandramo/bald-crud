...
func TestBuildSearchQuery_Wiring(t *testing.T) {
	// JSON 对象（值为带引号字面量）
	q := BuildSearchQuery(&storev1.PagingRequest{
		FilteringType: &storev1.PagingRequest_Query{Query: `{"status":"active"}`},
	})
	if q != `status:"active"` {
	...
	q = BuildSearchQuery(&storev1.PagingRequest{
		FilteringType: &storev1.PagingRequest_Query{Query: `[{"a":"1"},{"b":"2"}]`},
	})
	...
	q = BuildSearchQuery(&storev1.PagingRequest{
		FilteringType: &storev1.PagingRequest_Query{Query: `{"a":"x OR admin_field:1"}`},
	})
	...
	// 非法键（含元字符）整条丢弃
	q = BuildSearchQuery(&storev1.PagingRequest{
		FilteringType: &storev1.PagingRequest_Query{Query: `{"a(b":"1","c":"2"}`},
	})
	...
	// 非法/空输入 → 空串（match_all）
	if q := BuildSearchQuery(&storev1.PagingRequest{
		FilteringType: &storev1.PagingRequest_Query{Query: "not-json"},
	}); q != "" {
		t.Errorf("invalid query should be empty, got %q", q)
	}
	if q := BuildSearchQuery(&storev1.PagingRequest{}); q != "" {
...
func TestBuildSearchQuery_NonStringValues(t *testing.T) {
	q := BuildSearchQuery(&storev1.PagingRequest{
		FilteringType: &storev1.PagingRequest_Query{Query: `{"age":30,"active":true}`},
	})
