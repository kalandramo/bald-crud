...
func (ss StructuredSorting) BuildSelector(orders []*storev1.Sorting) (func(s *sql.Selector), error) {
	if len(orders) == 0 {
		return nil, nil
	}
	...
	return func(s *sql.Selector) {
		for _, order := range orders {
			...
			buildOrderBySelector(s, order.Field, order.GetDirection() == storev1.Sorting_DESC)
		}
	}, nil
}
...
// - defaultDesc: 默认是否降序
func (ss StructuredSorting) BuildSelectorWithDefaultField(orders []*storev1.Sorting, defaultOrderField string, defaultDesc bool) (func(s *sql.Selector), error) {
