...
func TestFilterExprExamples(t *testing.T) {
	t.Run("SimpleAND", func(t *testing.T) {
		...
		if fe.GetType() != storev1.ExprType_AND {
	...
	t.Run("SimpleOR", func(t *testing.T) {
		...
		if fe.GetType() != storev1.ExprType_OR {
	...
	t.Run("Mixed_A_AND_BorC", func(t *testing.T) {
		...
		orGroup := &storev1.FilterExpr{
			Type: storev1.ExprType_OR,
			Conditions: []*storev1.FilterCondition{
				{Field: "B", Op: storev1.Operator_EQ, ValueOneof: &storev1.FilterCondition_Value{Value: "2"}},
				{Field: "C", Op: storev1.Operator_EQ, ValueOneof: &storev1.FilterCondition_Value{Value: "3"}},
			},
		}
		fe := &storev1.FilterExpr{
			Type:       storev1.ExprType_AND,
			Conditions: []*storev1.FilterCondition{{Field: "A", Op: storev1.Operator_EQ, ValueOneof: &storev1.FilterCondition_Value{Value: "1"}}},
			Groups:     []*storev1.FilterExpr{orGroup},
		}
		...
		if fe.GetGroups()[0].GetType() != storev1.ExprType_OR {
	...
	t.Run("ComplexNested", func(t *testing.T) {
		...
		left := &storev1.FilterExpr{
			Type: storev1.ExprType_OR,
			Conditions: []*storev1.FilterCondition{
				{Field: "A", Op: storev1.Operator_EQ, ValueOneof: &storev1.FilterCondition_Value{Value: "a"}},
				{Field: "B", Op: storev1.Operator_EQ, ValueOneof: &storev1.FilterCondition_Value{Value: "b"}},
			},
		}
		rightInner := &storev1.FilterExpr{
			Type: storev1.ExprType_AND,
			Conditions: []*storev1.FilterCondition{
				{Field: "D", Op: storev1.Operator_EQ, ValueOneof: &storev1.FilterCondition_Value{Value: "d"}},
				{Field: "E", Op: storev1.Operator_EQ, ValueOneof: &storev1.FilterCondition_Value{Value: "e"}},
			},
		}
		right := &storev1.FilterExpr{
			Type: storev1.ExprType_OR,
			Conditions: []*storev1.FilterCondition{
				{Field: "C", Op: storev1.Operator_EQ, ValueOneof: &storev1.FilterCondition_Value{Value: "c"}},
			},
			Groups: []*storev1.FilterExpr{rightInner},
		}
		fe := &storev1.FilterExpr{
			Type:   storev1.ExprType_AND,
			Groups: []*storev1.FilterExpr{left, right},
		}
	...
	t.Run("SingleCondition", func(t *testing.T) {
		...
		fe := &storev1.FilterExpr{
			Type: storev1.ExprType_AND,
			Conditions: []*storev1.FilterCondition{
				{Field: "status", Op: storev1.Operator_EQ, ValueOneof: &storev1.FilterCondition_Value{Value: "active"}},
			},
		}
...
func TestBuildFilterSelectors_UnspecifiedExpr(t *testing.T) {
	...
	expr := &storev1.FilterExpr{
		Type: storev1.ExprType_EXPR_TYPE_UNSPECIFIED,
	}
...
func TestStructuredFilter_VariousConditions(t *testing.T) {
	...
	cases := []struct {
		...
		{"LIKE", storev1.Operator_LIKE, "pattern%", nil},
		{"ILIKE", storev1.Operator_ILIKE, "pattern%", nil},
		{"NOT_LIKE", storev1.Operator_NOT_LIKE, "pattern%", nil},
		{"IN", storev1.Operator_IN, "", []string{"a", "b"}},
		{"NIN", storev1.Operator_NIN, "", []string{"a", "b"}},
		{"IS_NULL", storev1.Operator_IS_NULL, "", nil},
		{"IS_NOT_NULL", storev1.Operator_IS_NOT_NULL, "", nil},
		{"BETWEEN", storev1.Operator_BETWEEN, "", []string{"1", "5"}},
		{"REGEXP", storev1.Operator_REGEXP, "regex", nil},
		{"IREGEXP", storev1.Operator_IREGEXP, "regex", nil},
		...
		{"JSON_CONTAINS", storev1.Operator_JSON_CONTAINS, `{"k":"v"}`, nil},
		{"ARRAY_CONTAINS", storev1.Operator_ARRAY_CONTAINS, "elem", nil},
		{"EXISTS", storev1.Operator_EXISTS, "subquery", nil},
...
func TestBuildFilterSelectors_JsonField(t *testing.T) {
			...
			Conditions: []*storev1.FilterCondition{
				{
					...
					ValueOneof: &storev1.FilterCondition_Value{Value: "v1"},
				},
			},
		}
		sels, err := sf.BuildSelectors(expr)
			...
			Conditions: []*storev1.FilterCondition{
				{
					...
					Op:         storev1.Operator_EQ,
					ValueOneof: &storev1.FilterCondition_Value{Value: "John"},
				},
			},
		}
		sels, err := sf.BuildSelectors(expr)
			...
			Conditions: []*storev1.FilterCondition{
				{
					...
					Op:         storev1.Operator_NEQ,
					ValueOneof: &storev1.FilterCondition_Value{Value: "inactive"},
				},
			},
		}
		sels, err := sf.BuildSelectors(expr)
			...
			Conditions: []*storev1.FilterCondition{
				{
					...
					Op:         storev1.Operator_CONTAINS,
					ValueOneof: &storev1.FilterCondition_Value{Value: "python"},
				},
			},
		}
		sels, err := sf.BuildSelectors(expr)
			...
			Conditions: []*storev1.FilterCondition{
				{
					...
					Op:         storev1.Operator_STARTS_WITH,
					ValueOneof: &storev1.FilterCondition_Value{Value: "admin"},
				},
			},
		}
		sels, err := sf.BuildSelectors(expr)
			...
			Conditions: []*storev1.FilterCondition{
				{
					...
					JsonPath:   trans.Ptr("status"),
					Op:         storev1.Operator_EQ,
					ValueOneof: &storev1.FilterCondition_Value{Value: "active"},
				},
			},
		}
		sels, err := sf.BuildSelectors(expr)
	...
	t.Run("JsonPathVsNormalField", func(t *testing.T) {
		...
		jsonPathExpr := &storev1.FilterExpr{
			Type: storev1.ExprType_AND,
			Conditions: []*storev1.FilterCondition{
				{
					Field:      "data",
					JsonPath:   trans.Ptr("name"),
					Op:         storev1.Operator_EQ,
					ValueOneof: &storev1.FilterCondition_Value{Value: "test"},
				},
			},
		}

		normalFieldExpr := &storev1.FilterExpr{
			Type: storev1.ExprType_AND,
			Conditions: []*storev1.FilterCondition{
				{
					Field:      "name",
...
func TestStructuredFilter_FieldAllowlist(t *testing.T) {
	...
	validExpr := &storev1.FilterExpr{
		Type: storev1.ExprType_AND,
		Conditions: []*storev1.FilterCondition{
			{
				Field:      "name",
				Op:         storev1.Operator_EQ,
				ValueOneof: &storev1.FilterCondition_Value{Value: "x"},
			},
		},
	}
	...
	invalidExpr := &storev1.FilterExpr{
		Type: storev1.ExprType_AND,
		Conditions: []*storev1.FilterCondition{
			{
				Field:      "password",
