<p align="center">
  <h1 align="center">bald-crud · 通用数据访问层工具库</h1>
  <p align="center">
...
go get github.com/kalandramo/bald-crud/gorm         # GORM
go get github.com/kalandramo/bald-crud/entgo         # Ent
go get github.com/kalandramo/bald-crud/mongodb       # MongoDB
go get github.com/kalandramo/bald-crud/clickhouse    # ClickHouse
go get github.com/kalandramo/bald-crud/doris         # Apache Doris
go get github.com/kalandramo/bald-crud/elasticsearch # Elasticsearch
go get github.com/kalandramo/bald-crud/opensearch    # OpenSearch
go get github.com/kalandramo/bald-crud/influxdb      # InfluxDB
```
...
func main() {
    ...
    page := uint32(1)
    pageSize := uint32(10)
    result, _ := repo.ListWithPaging(ctx, db, &storev1.PagingRequest{
        Page:     &page,
        PageSize: &pageSize,
    })
...
result, _ := repo.ListWithPaging(ctx, db, &storev1.PagingRequest{
    Page:     &page,
    PageSize: &pageSize,
    FilteringType: &storev1.PagingRequest_FilterExpr{
        FilterExpr: &storev1.FilterExpr{
            Type: storev1.ExprType_AND,
            Conditions: []*storev1.FilterCondition{
                {Field: "status", Op: storev1.Operator_EQ, Value: &storev1.FilterCondition_Value{Value: "active"}},
                {Field: "age", Op: storev1.Operator_GTE, Value: &storev1.FilterCondition_Value{Value: "18"}},
            },
        },
    },
})
...
| 特性 | bald-crud | 纯手写 Repository | 其他 CRUD 库 |
|------|---------|-------------------|-------------|
