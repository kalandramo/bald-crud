...
import (
	...
	"github.com/kalandramo/bald-crud/mongodb/mixin"
...
func TestInjectTenantFilter_TenantContextInjects(t *testing.T) {
	...
	if m["tenant_id"] != "t-7" {
		t.Errorf("tenant_id must be injected as t-7, got %v", m["tenant_id"])
	}
}
...
func TestEnforceOnScopedInstance_TenantContextSets(t *testing.T) {
	...
	if got == nil || *got != "t-7" {
		t.Errorf("tenant_id must be force-set to t-7, got %v", got)
	}
}
