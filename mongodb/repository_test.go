...
func TestRepository_ErrorBranches(t *testing.T) {
	...
	_, _, err := repoNilDB.ListWithPaging(ctx, &storev1.PagingRequest{})
	assert.Error(t, err)
	...
	_, _, err = repoEmptyColl.ListWithPaging(ctx, &storev1.PagingRequest{})
	assert.Error(t, err)
...
func TestRepository_CRUDAndList(t *testing.T) {
	...
	// List all
	all, _, err := repo.ListWithPaging(ctx, &storev1.PagingRequest{})
	assert.NoError(t, err)
	assert.Len(t, all, 3)
	ids := map[int]bool{}
	for _, d := range all {
	...
	final, _, err := repo.ListWithPaging(ctx, &storev1.PagingRequest{})
	assert.NoError(t, err)
