...
func (sf StructuredFilter) BuildSelectors(expr *storev1.FilterExpr) ([]func(*gorm.DB) *gorm.DB, error) {
...
func (sf StructuredFilter) buildFilterSelector(expr *storev1.FilterExpr) (func(*gorm.DB) *gorm.DB, error) {
	if expr == nil {
	...
	closure := func(db *gorm.DB) *gorm.DB {
		...
		sep := " AND "
		if expr.GetType() == storev1.ExprType_OR {
			sep = " OR "
		}
...
func (sf StructuredFilter) buildExpr(db *gorm.DB, expr *storev1.FilterExpr) ([]string, []any, error) {
	if expr == nil {
		return nil, nil, nil
	}
	...
	for _, g := range expr.GetGroups() {
		...
		sep := " AND "
		if g.GetType() == storev1.ExprType_OR {
			sep = " OR "
		}
...
func (sf StructuredFilter) condExprs(db *gorm.DB, conditions []*storev1.FilterCondition) ([]string, []any, error) {
	if len(conditions) == 0 {
		return nil, nil, nil
	}
	var frags []string
...
func (sf StructuredFilter) condExpr(db *gorm.DB, cond *storev1.FilterCondition) (string, []any, error) {
	if cond == nil {
		return "", nil, nil
	}
	val := ""
	switch cond.ValueOneof.(type) {
	case *storev1.FilterCondition_Value:
		val = cond.GetValue()
	default:
	}
...
func isSkippableEmpty(cond *storev1.FilterCondition, val string) bool {
	if val != "" {
		return false
	}
	switch cond.GetOp() {
	case storev1.Operator_IN, storev1.Operator_NIN, storev1.Operator_BETWEEN:
