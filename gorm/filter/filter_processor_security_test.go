...
func TestProcess_HostileFieldNeutralized(t *testing.T) {
	...
	for _, field := range hostile {
		...
		tx = poc.Process(tx.Model(&User{}), storev1.Operator_EQ, field, "v", nil)
	...
	for _, field := range hostile {
		expr := &storev1.FilterExpr{
			...
			Conditions: []*storev1.FilterCondition{
				{Field: field, Op: storev1.Operator_EQ, ValueOneof: &storev1.FilterCondition_Value{Value: "v"}},
			},
		}
...
func TestProcess_ValidFieldAndJsonExprStillWork(t *testing.T) {
	...
	tx := db.Session(&gorm.Session{DryRun: true})
	tx = poc.Process(tx.Model(&User{}), storev1.Operator_EQ, "Name", "bob", nil)
	var out []User
	if err := tx.Find(&out).Error; err != nil {
	...
	tx2 = poc.Process(tx2.Model(&User{}), storev1.Operator_EQ, expr, "1", nil)
	var out2 []User
...
func TestBuildSelectors_OrGroupActuallyFilters(t *testing.T) {
	...
	expr := &storev1.FilterExpr{
		...
		Conditions: []*storev1.FilterCondition{
			...
			{Field: "status", Op: storev1.Operator_EQ, ValueOneof: &storev1.FilterCondition_Value{Value: "disabled"}},
		},
	}
...
func TestBuildSelectors_OrGroupHostileFieldFailsClosed(t *testing.T) {
	...
	expr := &storev1.FilterExpr{
		Type: storev1.ExprType_OR,
		Conditions: []*storev1.FilterCondition{
			{Field: "id) OR (1=1 --", Op: storev1.Operator_EQ, ValueOneof: &storev1.FilterCondition_Value{Value: "1"}},
		},
	}
	sels, err := NewStructuredFilter().BuildSelectors(expr)
	if err != nil {
...
func TestBuildSelectors_EmptyValueSkipped(t *testing.T) {
	...
	expr := &storev1.FilterExpr{
		Type: storev1.ExprType_AND,
		Conditions: []*storev1.FilterCondition{
			{Field: "name", Op: storev1.Operator_EQ, ValueOneof: &storev1.FilterCondition_Value{Value: ""}},
		},
	}
	sels, err := NewStructuredFilter().BuildSelectors(expr)
	if err != nil {
	...
	expr2 := &storev1.FilterExpr{
		Type: storev1.ExprType_AND,
		Conditions: []*storev1.FilterCondition{
			{Field: "name", Op: storev1.Operator_CONTAINS, ValueOneof: &storev1.FilterCondition_Value{Value: ""}},
		},
	}
...
func TestBuildSelectors_InvalidFieldEmptyValueFailsClosed(t *testing.T) {
	...
	expr := &storev1.FilterExpr{
		Type: storev1.ExprType_AND,
		Conditions: []*storev1.FilterCondition{
			{Field: "idÿ' OR '1'='1", Op: storev1.Operator_EQ, ValueOneof: &storev1.FilterCondition_Value{Value: ""}},
		},
	}
	sels, err := NewStructuredFilter().BuildSelectors(expr)
	if err != nil {
