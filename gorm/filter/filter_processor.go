...
func (poc Processor) Process(db *gorm.DB, op storev1.Operator, field, value string, values []string) *gorm.DB {
	if db == nil {
		return db
	}
...
func requiresValue(op storev1.Operator) bool {
	switch op {
	case storev1.Operator_IS_NULL, storev1.Operator_IS_NOT_NULL,
		storev1.Operator_IN, storev1.Operator_NIN, storev1.Operator_BETWEEN:
		return false
	default:
		return true
	}
...
func (poc Processor) BuildExpression(db *gorm.DB, op storev1.Operator, field, value string, values []string) (string, []any, bool) {
